...
void main() {
  ...
  vec3 ramp = mix(mix(uColorStops[0], uColorStops[1], uv.x), uColorStops[2], uv.x * uv.y);
  float height = snoise(vec2(uv.x * 2.0 + uTime * 0.1, uTime * 0.25)) * 0.5 * uAmplitude;
  height = exp(height);
  float intensity = 0.55 * (uv.y * 2.0 - height + 0.2);
  float alpha = smoothstep(0.20 - uBlend * 0.5, 0.20 + uBlend * 0.5, intensity);
  fragColor = vec4(ramp * intensity * alpha, alpha);
}
`;
...
const DEFAULT_STOPS = ["#3d2412", "#c4a574", "#1a1208"];

/** Vendored from React Bits Aurora (DavidHDev/react-bits), simplified for Overnight. */
